...
class User(db.Entity):
    ...
    send_transactions = Set('Transaction', reverse='sender')
...
class Wallet(db.Entity):
    ...
    adress = Required(str, unique=True)
    send_transactions = Set('Transaction', reverse='sender_wallet')
    received_transactions = Set('Transaction', reverse='receiver_wallet')


class Transaction(db.Entity):
    id = PrimaryKey(int, auto=True)
    sender = Optional(User, reverse='send_transactions')
    sender_wallet = Optional(Wallet, reverse='send_transactions')
    receiver = Optional(User, reverse='received_transactions')
    receiver_wallet = Optional(Wallet, reverse='received_transactions')
    sender_adress = Optional(str)
    receiver_adress = Optional(str)
